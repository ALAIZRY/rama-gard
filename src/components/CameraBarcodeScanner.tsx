...
export const CameraBarcodeScanner: React.FC<CameraBarcodeScannerProps> = ({
  ...
  const startNativeScanner = async () => {
    ...
    try {
      ...
      if (lower.includes('cancel') || lower.includes('dismiss') || lower.includes('user')) {
        // Normal user cancellation.
      } else if (lower.includes('permission') || lower.includes('denied') || lower.includes('camera')) {
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-5 shadow-2xl dark:bg-slate-900">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-blue-600 p-3 text-white"><Camera size={22} /></div>
            <div>
              <h2 className="font-bold text-slate-900 dark:text-white">{title}</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">ماسح Android الأصلي</p>
            </div>
          </div>
          <button type="button" onClick={onClose} className="rounded-full p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="إغلاق"><X size={22} /></button>
        </div>
        ...
        {isLaunching && (
          <div className="mb-4 flex items-center gap-3 rounded-2xl bg-blue-50 p-4 text-blue-800 dark:bg-blue-950/40 dark:text-blue-200">
            <Camera size={20} className="animate-pulse" /><span className="text-sm font-medium">جاري فتح الكاميرا الأصلية...</span>
          </div>
        )}
        ...
        {error && (
          <div className="mb-4 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-200">
            <div className="mb-2 flex items-center gap-2 font-bold"><AlertCircle size={20} />مشكلة في الكاميرا</div>
            <p className="text-sm leading-6">{error}</p>
            <button type="button" onClick={() => void startNativeScanner()} className="mt-3 rounded-xl bg-red-600 px-4 py-2 text-sm font-bold text-white">إعادة المحاولة</button>
          </div>
        )}
        ...
        {lastCode && (
          <div className="mb-4 flex items-center gap-2 rounded-2xl bg-green-50 p-3 text-green-800 dark:bg-green-950/30 dark:text-green-200">
            <Check size={20} /><span className="text-sm font-bold">آخر باركود: {lastCode}</span>
          </div>
        )}

        <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
          <label className="mb-2 block text-sm font-bold text-slate-700 dark:text-slate-200">إدخال الباركود يدويًا</label>
          <div className="flex gap-2">
            <input value={manualInput} onChange={(e) => setManualInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleManualSubmit(); }} inputMode="numeric" autoComplete="off" placeholder="اكتب أو الصق الباركود" className="min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-3 py-3 text-left outline-none focus:border-blue-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
            <button type="button" onClick={handleManualSubmit} className="rounded-xl bg-blue-600 px-4 py-3 font-bold text-white">إدخال</button>
          </div>
        </div>
          ...
          <div className="mt-4 max-h-40 overflow-auto rounded-2xl bg-slate-50 p-3 dark:bg-slate-800">
            ...
            {history.slice(0, 8).map((item) => (
              <div key={item.id} className="flex items-center justify-between border-b border-slate-200 py-2 last:border-0 dark:border-slate-700">
                <span className="font-mono text-sm font-bold" dir="ltr">{item.code}</span><span className="text-xs text-slate-500">{item.timestamp}</span>
              </div>
            ))}
          </div>
        )}

        <button type="button" onClick={onClose} className="mt-4 w-full rounded-xl border border-slate-300 px-4 py-3 font-bold text-slate-700 dark:border-slate-600 dark:text-slate-200">إغلاق</button>
      </div>
    </div>
  );
};
